...
func (h *Handler) getReportProduct(writer http.ResponseWriter, request *http.Request) {
	...
	for _, item := range response.GetDaily() {
		...
		partial = partial || item.GetStatus() == "partial"
	}
...
func (h *Handler) createReportCashFlow(writer http.ResponseWriter, request *http.Request) {
	...
	if body.OccurredAt != "" {
		...
		if flowDate == "" {
			flowDate = parsed.Format(time.DateOnly)
		}
	}
	response, err := h.report.CreateCashFlow(ctx, &reportv1.CreateCashFlowRequest{
		Token: token, ProductId: productID, FlowDate: flowDate,
		AmountUsd: amount, FlowType: flowType, Note: body.Note,
		Confirmed: body.Confirmed || body.Status == "confirmed", OccurredAt: occurredAt,
	})
	if err != nil {
		writeReportError(writer, err)
		return
	}
	writeJSON(writer, http.StatusCreated, map[string]any{"data": reportCashFlowJSON(response.GetCashFlow())})
}
...
func reportDailyJSON(item *reportv1.DailySnapshot) map[string]any {
	return map[string]any{
		...
		"absoluteReturn": item.GetAbsoluteReturn(), "dailyReturn": item.GetReturnRate(),
		...
		"returnRate": item.GetReturnRate(), "sampleCount": item.GetSampleCount(),
		"status":      map[bool]string{true: "provisional", false: "final"}[item.GetStatus() == "partial"],
		"partial":     item.GetStatus() == "partial",
