...
class AsterVenueAdapter final : public VenueAdapter {
 public:
  struct SymbolState {
  ...
  AsterVenueAdapter(utils::md::ProductType product,
                      std::size_t maximum_levels)
  ...
  bool build_subscription_batches(
    ...
    for (const auto &request : requests) {
      if (!valid_utf8_symbol(request.venue_symbol)) {
        error = "invalid Aster venue symbol";
        return false;
      }
      ...
      if (request.ticker) {
        if (!request.ticker_channel.empty() &&
          ...
          error = "unsupported Aster ticker channel";
          return false;
        }
      ...
      if (request.orderbook) {
        if (!request.orderbook_channel.empty() &&
          ...
          error = "unsupported Aster orderbook channel";
          return false;
        }
        const auto interval =
    ...
    if (topics.empty()) {
      error = "at least one Aster stream must be requested";
      return false;
    }
    batches.clear();
  ...
  bool build_unsubscription_batches(
    ...
    for (auto &batch : batches) {
      ...
      if (operation == std::string::npos) {
        error =
            "Aster subscription cannot be converted to unsubscribe";
        batches.clear();
        return false;
      }
  ...
  bool parse_ws(std::string_view json, NormalizedEvent &event,
    ...
    try {
      ...
      if (!code.error()) {
        ...
        error = "Aster subscription rejected";
      ...
      if (state == nullptr || !copy_symbol(symbol, event)) {
        error = "Aster event references an unknown symbol: ";
        error.append(symbol);
        return false;
      }
      ...
      if (event_name == "depthUpdate") {
        ...
        // Live Aster Spot and Perpetual feeds both publish `pu`. Spot `U`
        // may skip values while `pu` still links exactly to the prior `u`.
        ...
        if (state->depth.bids.size() > event.bids.capacity() ||
          ...
          error = "Aster depth update exceeds normalized event capacity";
          return true;
        }
  ...
  bool enrich_discovery_turnover(
    ...
    (void)metadata;
    error =
        "Aster 24h turnover parsing requires simdjson support";
    return false;
#else
    try {
      ...
      if (entries_result.error()) {
        ...
        if (!object_result.error() &&
          ...
          error = "Aster 24h ticker request failed";
          auto message =
          ...
          error = "Aster 24h ticker response is not an array";
        }
        return false;
      }
      ...
      for (auto raw_entry : entries_result.value()) {
        ...
        if (matched[index]) {
          error = "duplicate Aster 24h ticker symbol: ";
          error.append(symbol);
          return false;
        }
        ...
        if (turnover_result.error() ||
            !decimal_to_turnover(turnover_result.value(),
                                 turnovers[index])) {
          error = "invalid Aster 24h quote turnover for symbol: ";
          error.append(symbol);
          return false;
        }
        matched[index] = true;
  ...
  HttpRequestSpec snapshot_request(std::string_view venue_symbol,
    ...
    std::string target(profile_ == Profile::Spot ? "/api/v3/depth"
                                                  : "/fapi/v3/depth");
  ...
  bool parse_snapshot(std::string_view json, std::string_view venue_symbol,
    ...
    if (state == nullptr || !copy_symbol(venue_symbol, event)) {
      error = "Aster snapshot references an unknown symbol";
      return false;
    }
    ...
    if (!rest_parser_.parse_depth(profile_, json, state->metadata,
      ...
      error = "Aster snapshot symbol=" + std::string(venue_symbol) +
              ": " + error;
      return false;
    }
  ...
  bool parse_metadata_values(
    ...
    for (std::size_t index = 0; index < requests.size(); ++index) {
      ...
      if (parsed.price_scale < 0 || parsed.quantity_scale < 0) {
        error = "invalid Aster instrument scale";
        return false;
      }
      ...
      if (instrument.tick_size <= 0 || instrument.lot_size <= 0) {
        error = "invalid Aster instrument increments";
        return false;
      }
...
make_aster_adapter(utils::md::ProductType product,
  ...
  return std::make_unique<AsterVenueAdapter>(
