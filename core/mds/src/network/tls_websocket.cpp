...
int TlsSession::handshake() noexcept {
  last_ssl_result_ = ssl_ ? SSL_connect(ssl_) : -1;
  return last_ssl_result_;
}
...
int TlsSession::read_some(std::span<const std::byte> &data) noexcept {
  ...
  last_ssl_result_ = SSL_read(ssl_, receive_buffer_.data(), length);
  if (last_ssl_result_ > 0) {
    data = {receive_buffer_.data(),
            static_cast<std::size_t>(last_ssl_result_)};
    return last_ssl_result_;
  }
  const int error = SSL_get_error(ssl_, last_ssl_result_);
  return error == SSL_ERROR_ZERO_RETURN ? 0 : -error;
}
...
int TlsSession::write(std::span<const std::byte> data) noexcept {
  ...
  const auto length = static_cast<int>(
      std::min(data.size(),
               static_cast<std::size_t>(std::numeric_limits<int>::max())));
  last_ssl_result_ = SSL_write(ssl_, data.data(), length);
  if (last_ssl_result_ > 0) {
    return last_ssl_result_;
  }
  const int error = SSL_get_error(ssl_, last_ssl_result_);
