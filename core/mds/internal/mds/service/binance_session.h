...
struct BinanceSessionOptions {
  ...
  std::size_t ladder_levels_per_side{8192};
  std::size_t max_ladder_levels_per_side{16384};
...
public:
  ...
  BinanceSession(network::EpollLoop &loop, network::SharedSslContext tls,
  ...
  [[nodiscard]] static bool snapshot_allowed(
      network::WebSocketClientState state) noexcept {
    return state == network::WebSocketClientState::Open;
  }
...
private:
  ...
  network::EpollLoop &loop_;
  network::SharedSslContext tls_;
  BinanceSessionOptions options_;
  network::WebSocketClient websocket_;
  network::HttpClient metadata_http_;
  network::HttpClient snapshot_http_;
  ...
  std::unique_ptr<publish::TickerOrderBookPublishers> publishers_;
...
public:
  SessionManager();
  explicit SessionManager(network::SharedSslContext tls);
